                 ...
                 int first_cmd)
{
    ...
    for (i = first_cmd; i < argc; i++) {
        ...
        if (par->prop_name == "Synaptics Off") {
            /*Change the LED status property as same as Touchpad Off*/
            prop = XInternAtom(dpy, SYNAPTICS_PROP_LED_STATUS, True);
            XChangeDeviceProperty(dpy, dev, prop, type, format,
                                  PropModeReplace, data, nitems);
            /*Change the double tap led status if the touchpad is switch on*/
            prop = XInternAtom(dpy, SYNAPTICS_PROP_LED_DOUBLE_TAP_STATUS, True);
            if ((XGetDeviceProperty (dpy, dev, prop, 0, 1, False, type, &type,
