...
static XDevice *dev;
static Atom touchpad_off_prop , led_status, touchpad_off_double_tap_status;
...
toggle_touchpad(Bool enable)
{
    ...
    /* Enable or disable the touchpad only if the double tap led status is
     * disable*/
    if ((XGetDeviceProperty (display, dev, touchpad_off_double_tap_status, 0, 1, False,
                             XA_INTEGER, &real_type, &real_format, &nitems,
                             &bytes_after, &val) == Success) && (val[0] == 0)) {
        ...
        /*Change the LED status property as same as Touchpad Off*/
        if (data == TappingOff)
            data = 1;
        XChangeDeviceProperty(display, dev, led_status, XA_INTEGER, 8,
                              PropModeReplace, &data, 1);
        XFlush(display);
    }
}
...
dp_get_device(Display * dpy)
{
    ...
    touchpad_off_double_tap_status = XInternAtom(dpy, SYNAPTICS_PROP_LED_DOUBLE_TAP_STATUS, True);
    led_status = XInternAtom(display, SYNAPTICS_PROP_LED_STATUS, True);
