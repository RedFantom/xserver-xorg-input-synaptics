...
set_softbutton_areas_option(InputInfoPtr pInfo, char *option_name, int offset)
{
    ...
    double values[8]; /* use double instead of int to use float as parameter*/
    int values_int[8];
...
/* To use same parameter for right_left and middle_right
 * If right_left = middle_right, soustract 1 to middle_right
 */
    if ((values_int[0] > 0) && (values_int[0] == values_int[5]))
        values_int[5] = values_int[5] - 1;

    if (!SynapticsIsSoftButtonAreasValid(values_int))
        goto fail;
    
    memcpy(pars->softbutton_areas[0], values_int, 4 * sizeof(int));
    memcpy(pars->softbutton_areas[1], values_int + 4, 4 * sizeof(int));
...
set_default_parameters(InputInfoPtr pInfo)
{
    ...
    pars->led_status = xf86SetBoolOption(opts, "LEDStatus", FALSE);
    pars->led_double_tap = xf86SetBoolOption(opts, "LEDDoubleTap", TRUE);
    pars->led_double_tap_status = xf86SetBoolOption(opts, "LEDDoubleTapStatus", FALSE);
...
handle_toggle_led(InputInfoPtr pInfo, struct SynapticsHwState *hw, int finger)
{
    ...
    if (priv->led_tapped) {
        /* double-tapped? */
        diff = TIME_DIFF(priv->led_tap_millis + 2 * para->tap_time_2, hw->millis);
        if (diff >= 0) {
            ...
            /*Change the double tap status*/
            SynapticsDoubleTapStatus(pInfo->dev, para->touchpad_off);
            priv->led_tapped = FALSE;
        }
    } else
                    ...
                    Bool inside_active_area)
{
    ...
    if ((para->touchpad_off == TOUCHPAD_OFF || priv->finger_state == FS_BLOCKED) &&
        !(para->has_led && para->led_double_tap))
        return delay;
