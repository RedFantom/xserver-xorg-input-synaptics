...
Atom prop_led_double_tap_status = 0;
...
InitDeviceProperties(InputInfoPtr pInfo)
{
    ...
    prop_led_double_tap_status = InitAtom(pInfo->dev, SYNAPTICS_PROP_LED_DOUBLE_TAP_STATUS, 8, 1, &para->led_double_tap_status);
...
void SynapticsToggleOffProperty(DeviceIntPtr dev, Bool off)
{
        ...
        XIChangeDeviceProperty(dev, prop_led_status, XA_INTEGER, 8,
                               PropModeReplace, 1, &val, FALSE);
}

/*Function to change the double tap led status*/
void SynapticsDoubleTapStatus(DeviceIntPtr dev, Bool off)
{
        uint8_t val;
        ...
        if (!prop_off)
                return;
        val = off;
        XIChangeDeviceProperty(dev, prop_led_double_tap_status, XA_INTEGER, 8,
                               PropModeReplace, 1, &val, FALSE);
}
int
